...
public class TVServiceImpl implements TVService {
	...
	public JSONObject insertPreFile(HttpServletRequest request, @RequestBody PreFileInfo file) {
		...
		try {
			...
			if (file.getPreFileId() == 0) {
				...
				opt.setOptRemark(sessionObj.getString("roleName") + "-" + sessionObj.getString("userName")
						+ ",添加电视目录，名称《" + file.getPreFileName().split(",")[0] + "》");
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public JSONObject deleteTv(HttpServletRequest request, TVListInfo tv) {
		JSONObject obj = new JSONObject();
		try {
			JSONObject sessionObj = (JSONObject) request.getSession().getAttribute(KeyWord.USERSESSION);
			tv.setValid(ValidEnum.VALID.getValidStatus());
			...
			TVListInfo tt= tVMapper.findTVById(tv);
			OperateInfo opt1 = new OperateInfo();
			opt1.setOptType("update");
			opt1.setOptUserId(sessionObj.getLong("userId"));
			opt1.setOptName("删除电视");
			opt1.setOptRemark(sessionObj.getString("roleName") + "-" + sessionObj.getString("userName") + ",删除电视，名称《"
					+ tt.getTvName()+"》"+"--第"+tt.getTvLevel()+"集");
			opt1.setTypeId(tv.getPreFileId());
