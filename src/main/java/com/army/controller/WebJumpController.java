...
public class WebJumpController {
	...
	public JSONObject deleteVedio(HttpServletRequest request, VedioInfo imgs) throws Exception {
		imgs.setCreateName(GetUserInfo.getUserName(request));
		JSONObject obj = moiveService.deleteVedio(request,imgs);
		return obj;
	}
	
	@RequestMapping("/manager/deleteImg")
	public JSONObject deleteImg(HttpServletRequest request, ImgsHisInfo imgs) throws Exception {
		imgs.setCreateName(GetUserInfo.getUserName(request));
		JSONObject obj = operateService.deleteImg(imgs);
		return obj;
	}
	
	@RequestMapping("/manager/openfile")
	public JSONObject openfile() throws Exception {

		JSONObject obj = new JSONObject();
		obj.put("file", Count.getHisCount().split("kkbb")[1]);
		obj.put("size", Count.getHisCount().split("kkbb")[0]);

		return obj;
	}
	
	@RequestMapping("/manager/loadImg")
	public JSONObject loadImg(HttpServletRequest request,ImgsHisInfo tv) throws Exception {
		tv.setCreateName(GetUserInfo.getUserName(request));
		JSONObject obj = operateService.loadImg(tv);
	...
	@RequestMapping("/manager/findAllImgs")
	public JSONArray findAllImgs(HttpServletRequest request, ImgsHisInfo imgs) throws Exception {
		imgs.setCreateName(GetUserInfo.getUserName(request));
		JSONArray arr = operateService.findAllImgs(imgs);
	...
	public JSONObject getCount() {
		JSONObject obj = new JSONObject();
		obj.put("record", Count.getCount());
		obj.put("today", Count.getToday());
		return obj;
	}
